...
const stats = [
  ...
  { value: "9+", label: "Professional Courses" },
  { value: "233", label: "University Code" },
...
const courses = [
  {
    ...
    duration: "3 Years",
    seats: "40 seats",
    desc: "A comprehensive undergraduate program focused on software development, systems analysis, and modern computing technologies.",
    image: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&w=800&q=80",
    tags: ["Undergraduate", "Full-Time"]
  },
  {
    icon: GraduationCap,
    title: "P.G.D.C.A",
    full: "Post Graduate Diploma in Computer Application",
    duration: "1 Year",
    seats: "40 seats",
    desc: "An intensive postgraduate diploma offering advanced training in applied computer sciences and professional IT skills.",
  ...
  },
  {
    icon: BookOpen,
    title: "D.C.A",
    full: "Diploma in Computer Applications",
    duration: "1 Year",
    seats: "25 seats",
    desc: "A foundational diploma course providing essential knowledge in computer applications for career-ready graduates.",
    image: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=800&q=80",
    tags: ["Diploma", "Foundation"]
  },
];
...
const heroImages = [
  "/gtbptefrontroad.png",
  "/gtbptecollegebuilding.png",
  "/gtbptecollegebuilding2.png"
];

export default function HomePage() {
  const mainRef = useRef<HTMLElement>(null);
  const [currentSlide, setCurrentSlide] = useState(0);
  const [previousSlide, setPreviousSlide] = useState(heroImages.length - 1);
...
gsap.fromTo(
  ...
  return (
    <main ref={mainRef}>
{/* ── Hero ─────────────────────────────────────────── */}
<section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-transparent -mt-[120px] pt-[120px]">
...
{/* ✅ MOBILE TEXT (BOTTOM + VISIBLE) */}
<div className="absolute inset-0 z-20 flex items-end md:hidden pb-20">
  ...
  <div
    key={currentSlide}
   className="mobile-hero-text w-full px-6 pb-4 text-left"
  >
    {currentSlide === 0 && (
      <h1 className="text-2xl font-bold leading-snug text-white drop-shadow-lg">
        Welcome to <br />
        <span className="text-gold font-extrabold">
          GTB College of Professional & Teaching Education
        </span>
      </h1>
    )}
          ...
          <div className="lg:col-span-5 relative gsap-fade-up">
            ...
            <div className="absolute -bottom-10 -right-10 bg-navy p-8 md:p-10 z-20 hidden md:block rounded-sm shadow-2xl">
              <div className="text-gold font-serif text-6xl mb-2 flex items-end gap-2">
                <span className="animate-number" data-target="233">0</span><span className="text-xl pb-1.5 font-sans font-semibold uppercase tracking-widest">Code</span>
              </div>
          ...
          <div className="lg:col-span-7 lg:pl-10">
            ...
            <div className="prose prose-lg text-gray-600 font-sans mb-10 gsap-stagger-item">
              <p className="first-letter:text-6xl first-letter:font-serif first-letter:text-gold first-letter:float-left first-letter:mr-3 first-letter:leading-none">
                Established In the region of Chhattisgarh, which stands as a witness to India’s glorious past, the Bilaspur Model Education Society, Bilaspur, took a significant initiative in the field of education by establishing the GTB College of Professional & Technical Education in 2010.
              </p><br/>
              <p>
                The primary objective of the institution is not only to provide education but to impart true knowledge. It aims to elevate the thinking and perspective of students. With this vision, it has been resolved that the college will focus exclusively on job-oriented and professional courses, so that students can be well-prepared to perform effectively in the challenges of life.
              </p>
            </div>
      ...
      <section className="bg-white py-20 lg:py-32 px-4 md:px-8 overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          {/* <div className="grid md:grid-cols-3 gap-8 gsap-stagger-container">
            {courses.map((c, idx) => (
              <div key={idx} className="group flex flex-col bg-cream border border-gray-200 rounded-sm overflow-hidden hover:shadow-2xl hover:shadow-navy/5 transition-all duration-500 gsap-stagger-item">
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 flex gap-2">
                    {c.tags.map(tag => (
                      <span key={tag} className="px-3 py-1 bg-white/90 backdrop-blur-sm text-navy font-sans text-xs font-bold uppercase tracking-wider rounded-sm">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="p-8 flex-1 flex flex-col">
                  <h3 className="font-serif text-3xl text-navy mb-2">{c.title}</h3>
                  <p className="font-sans text-sm font-semibold text-gold mb-4">{c.full}</p>
                  <p className="font-sans text-gray-600 mb-6 leading-relaxed flex-1">
                    {c.desc}
                  </p>

                  <div className="flex items-center justify-between border-t border-gray-200 pt-6 mt-auto">
                    <div className="flex gap-4 text-sm font-sans font-medium text-gray-500">
                      <span className="flex items-center gap-1.5"><CalendarDays size={16} className="text-gold" /> {c.duration}</span>
                      <span className="flex items-center gap-1.5"><Users size={16} className="text-gold" /> {c.seats}</span>
                    </div>
                    <Link href="/academics" className="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-navy group-hover:bg-gold group-hover:border-gold group-hover:text-white transition-colors">
                      <ArrowRight size={18} />
                    </Link>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-2 gap-4 h-[600px] gsap-stagger-item">
            <div className="space-y-4">
              <div className="relative h-2/5 w-full rounded-sm overflow-hidden">
                <Image src="https://plus.unsplash.com/premium_photo-1683887034552-4635692bb57c?q=80&w=2069&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" alt="Campus Life" fill className="object-cover" />
              </div>
              <div className="relative h-3/5 w-full rounded-sm overflow-hidden">
                <Image src="https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?auto=format&fit=crop&w=600&q=80" alt="Library" fill className="object-cover" />
              </div>
            </div>
            <div className="space-y-4 pt-12">
              <div className="relative h-3/5 w-full rounded-sm overflow-hidden">
                <Image src="https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=600&q=80" alt="Computer Lab" fill className="object-cover" />
      ...
      <section className="py-24 px-4 md:px-8 bg-cream text-center overflow-hidden relative">
        ...
        <div className="max-w-3xl mx-auto gsap-stagger-container relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-4 gsap-stagger-item">
            <Link href="/contact" className="px-8 py-4 bg-white border border-gray-300 text-navy font-sans font-bold rounded-sm hover:bg-gray-50 transition-colors flex items-center gap-2">
